  ...
  return (
    <main className="h-screen overflow-hidden bg-[radial-gradient(circle_at_top,#dcfce7_0,#eff6ff_40%,#f8fafc_100%)] p-3 md:p-5">
      <div className="mx-auto flex h-full max-w-[1700px] flex-col">
        <header className="mb-4 rounded-2xl border border-slate-300 bg-white/90 p-4 shadow-sm backdrop-blur md:p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">
            UNMSM · INGENIERIA DE SOFTWARE
          </p>
          <h1 className="mt-2 text-2xl font-bold text-slate-900 md:text-3xl">Constructor de Horarios</h1>
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            Sube tu PDF, elige ciclo, arma manualmente o genera horarios sin cruces y guardalos en localStorage.
          </p>
        </header>
        ...
        <div className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[220px_minmax(0,1fr)_320px]">
          <aside className="order-2 min-h-0 space-y-4 overflow-hidden xl:order-1">
            <PDFUploader onFileSelected={parsePdf} loading={loadingPdf} compact={Boolean(data)} />
          ...
          <section className="order-1 min-h-0 space-y-4 overflow-hidden xl:order-2">
            ...
            {!data && !loadingPdf && (
              <p className="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-12 text-center text-sm text-slate-600">
                Aun no hay datos. Sube un PDF para empezar.
              </p>
            )}
